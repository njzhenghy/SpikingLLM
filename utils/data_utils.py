...
def get_hellaswag(tokenizer, train_size, val_size, seed, seqlen, test_only):
    print("get_hellaswag")
    traindata = load_dataset("Rowan/hellaswag", split="train", trust_remote_code=True)
    testdata = load_dataset('Rowan/hellaswag', split="validation", trust_remote_code=True)
    traindata = traindata.shuffle(seed=seed) 

    def build_full_text(data):
        lines = []
        for ex in data:
            context = ex['ctx_a'] + ex['ctx_b']
            ending = ex['endings'][int(ex['label'])]
            lines.append(context + " " + ending)
        return "\n\n".join(lines)

    test_text = build_full_text(testdata)
    testenc = tokenizer(test_text, return_tensors='pt')
    if test_only:
        return testenc
    train_text = build_full_text(traindata)
    ...
    for _ in range(train_size):
        i = random.randint(0, int(total_len * val_sample_ratio) - seqlen - 1)
        j = i + seqlen
    ...
    for _ in range(val_size):
        i = random.randint(int(total_len * val_sample_ratio), total_len - seqlen - 1)
        j = i + seqlen
        inp = trainenc.input_ids[:, i:j]
        tar = inp.clone()
        tar[:, :-1] = -100
...
def get_pile(tokenizer, train_size, val_size, seed, seqlen, test_only=False):
    print("get_pile")
...
def get_redpajama(tokenizer, train_size, val_size, seed, seqlen, test_only=False):
    print("get_redpajama")
    try:
...
def get_loaders(
    ...
    if 'wikitext2' in name:
        ...
        return get_redpajama(tokenizer,train_size,val_size,seed,seqlen,test_only)
    elif 'pile' in name:
        return get_pile(tokenizer,train_size,val_size,seed,seqlen,test_only)
    elif 'hellaswag' in name:
        return get_hellaswag(tokenizer,train_size,val_size,seed,seqlen,test_only)
    else:
...
def test_ppl(args, model, tokenizer,prefixed_key_values=None, datasets=['wikitext2']):
    ...
    for dataset in datasets:
        ...
        if "c4" in dataset or "redpajama" in dataset or "pile" in dataset:
            testenc = testloader
        else:
...
class SpikeBlockTrainDataset(Dataset):
    def __init__(self, T, size, seqlen, hidden_size, batch_size, dtype, cache_path='./cache/', off_load_to_disk=False):
        self.T = T
        self.size = size
        self.seqlen = seqlen
        self.hidden_size = hidden_size
        ...
        if self.off_load_to_disk:
            ...
            self.data = torch.zeros((self.size//self.batch_size, self.T*self.batch_size, self.seqlen, self.hidden_size), dtype=self.dtype)

    def _initialize_data_on_disk(self):
        for idx in range(self.size//self.batch_size):
            tensor = torch.zeros((self.T*self.batch_size, self.seqlen, self.hidden_size), dtype=self.dtype)
            filepath = self._get_file_path(idx)
            torch.save(tensor, filepath)
...
def copy_block_dataset_to_spike(T, dataset:BlockTrainDataset, cache_dir=None):
    new_cache_path = None
    if dataset.off_load_to_disk:
        old_cache_path = dataset.cache_path
        while new_cache_path is None or os.path.exists(new_cache_path):
            ...
            time.sleep(1) # avoid same flag
        shutil.copytree(old_cache_path, new_cache_path)
    new_dataset = SpikeBlockTrainDataset(T, dataset.size, dataset.seqlen,
                                    dataset.hidden_size, dataset.batch_size,
                                    dataset.dtype, new_cache_path,
                                    dataset.off_load_to_disk)
    if not dataset.off_load_to_disk:
        for index, data in enumerate(dataset):
            new_dataset.update_data(index, data.repeat(T, 1, 1)/T)
    return new_dataset
