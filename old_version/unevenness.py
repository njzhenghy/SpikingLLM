...
from utils.snn_utils import wrap_to_snn_model, replicate_past_key_values, get_spike_config, snn_wrap_to_quant_model
from utils.block_calibration import calibration
...
def main():
    ...
    parser.add_argument("--spike_model_path", type=str, help="model path of quantized model")
    ...
    quant_config = load_json_as_namespace(os.path.join(args.spike_model_path, 'spike_config.json'))
    ...
    with init_empty_weights():
        spike_model = AutoModelForCausalLM.from_pretrained(args.spike_model_path, config=config, device_map='cpu', torch_dtype=torch.float16, trust_remote_code=True)
    ...
    load_checkpoint_in_model(quant_model, checkpoint=args.spike_model_path, device_map=device_map, dtype=torch.float16, offload_folder=f'/cache/{username}/offload')
