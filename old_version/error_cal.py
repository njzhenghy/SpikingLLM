...
from utils.block_calibration_copy import calibration
...
def main():
    ...
    parser.add_argument("--quant_model_path", type=str, help="model path")
    ...
    if spike_config.set_prefixed_tokens:
        spike_prefixed_key_values = torch.load(os.path.join(args.spike_model_path, 'spike_prefixed_key_values.pth'))
    else:
    ...
    config_spike = AutoConfig.from_pretrained(args.spike_model_path,trust_remote_code=True)
    ...
    with init_empty_weights():
        ...
        spike_model = AutoModelForCausalLM.from_pretrained(args.quant_model_path, config=config_spike, device_map='cpu', torch_dtype=torch.float16, trust_remote_code=True)

    for param in model.parameters():
        param.requires_grad = False
    for param in model.parameters():
        spike_model.requires_grad = False
    ...
    for layer in layers:
        rotation_utils.add_qk_rotation_wrapper_after_function_call_in_forward(layer.self_attn, rope_function_name, config=model.config, online_had=quant_config.qk_online_had)
    ...
    if spike_config.wbits < 16:
        logger.info('init weight quantizer')
        init_weight_quantizer(spike_config, spike_model, logger=logger, minmax_init=False)

    # init input quantizer
    if spike_config.input_bits < 16:
        logger.info('init input quantizer')
        init_input_quantizer(spike_config, spike_model, logger=logger, minmax_init=False)

    if spike_config.output_bits < 16:
        logger.info('init output quantizer')
        init_out_quantizer(spike_config, spike_model, logger=logger, minmax_init=False)
    print("Loading pre-computed quantized weights...")

    wrap_to_snn_model(spike_model, spike_config.T, spike_config.avg_neuron)
    ...
    load_checkpoint_in_model(model, checkpoint=args.quant_model_path, device_map=device_map, dtype=torch.float16, offload_folder=f'/cache/{username}/offload')
    load_checkpoint_in_model(spike_model, checkpoint=args.spike_model_path, device_map=device_map, dtype=torch.float16, offload_folder=f'/cache/{username}/offload')
    dispatch_model(model, device_map=device_map)
    dispatch_model(spike_model, device_map=device_map)
    spike_model = calibration(model, spike_model, prefixed_key_values, spike_prefixed_key_values, args, trainloader, valloader, logger)
