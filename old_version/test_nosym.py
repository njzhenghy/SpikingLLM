...
x_s = x.add(round_zero_point.mul(scale))
x_int = torch.clamp(x_s, min_val, max_val)
x_int = x_int.add(v_0.detach())
x_int = floor_ste(x_int / scale)    # +0.5的技巧配合floor模拟（4）中的四舍五入计算，确保与论文中的公式（11）使用的floor对应
...
for T in T_list:
    ...
    x_reshaped = X.reshape(T, bs, n, -1, group_size)
    v = torch.ones_like(x_reshaped[0, ...]).mul(scale)*0.5
    v_th = scale / (T * L)  # 先不用管
    I_z = round_zero_point / T
    I_z = I_z.mul(scale)    # 根据Theorem 3求出 I_z
