...
def retrain(args, mode=None):
    ...
    arch = torch.load(f'/arch_dir/{args.model_name}-T-{args.T}-grains-{args.num_grains}/search_arch.pth', map_location="cuda:0")
    ...
    for i in range(args.start, args.end):
        ...
        # base opertaions
        for bk in base_keys:
            key_name = f"model.layers.{i}.{bk}"

            best_error_baseline = 0
            
            if args.genotype is None:
                _, genotype = arch[key_name]
            else:
                genotype = args.genotype
            ...
            if "softmax" not in key_name and "silu_Identity" not in key_name:
                fs_neuron_baseline = LASNeuron(T=args.T, tau=tau_value).to(args.device)
                fs_neuron_Ours = FSNeuronDecoupled(T=args.T, num_grains=args.num_grains,
                                genotype=genotype, tau=tau_value).to(args.device)
    ...
    torch.save(output_dict, f'./retrain_dir/{args.model_name}-T-{args.T}-grains-{args.num_grains}/retrain.pth')
    print("save success")
